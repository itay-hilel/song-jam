...
export function createSessionRoutes(app: Application): Router {
  ...
  router.post('/', async (req, res) => {
      try {
          const context: Context = { timestamp: new Date() };
          const sessionData: Session = req.body;
          const newSession = await sessionService.create(context, sessionData);
  ...
  router.post('/:sessionId/join', async (req, res) => {
      try {
          ...
          const updatedSession = await sessionService.join(context, sessionId, participant);
  ...
  router.post('/:sessionId/song-line', async (req, res) => {
      try {
          ...
          const updatedSession = await sessionService.addSongLine(context, sessionId, line);
          res.status(200).json(updatedSession);
      } catch (error) {
  ...
  router.put('/:sessionId/song-lines/reorder', async (req, res) => {
      try {
          ...
          const updatedSession = await sessionService.reorderLines(context, sessionId, lineIds);
          res.status(200).json(updatedSession);
      } catch (error) {
  ...
  router.post('/:sessionId/export', async (req, res) => {
      try {
          const context: Context = { timestamp: new Date() };
          const { sessionId } = req.params;
          const exportResult = await sessionService.exportToSono(context, sessionId);
